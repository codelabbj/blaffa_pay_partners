...
function BettingDepositContent() {
  ...
  const handlePlatformSelect = (platformUid: string) => {
    if (platformUid) {
      router.push(`/dashboard/betting/deposit?platform=${platformUid}`)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true)
    try {
      const result: TransactionCreateResponse = await createDeposit({
  ...
  const isAmountValid = () => {
    if (!platform || !formData.amount) return true
        ...
        <div className="bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl rounded-2xl md:rounded-3xl border border-white/20 dark:border-gray-700/50 shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <p className="text-sm md:text-base text-gray-500 dark:text-gray-400">
              Choisissez la plateforme sur laquelle vous souhaitez effectuer un dépôt
            </p>
          </div>
                  ...
                  <CardContent className="p-4 md:p-6">
                    <div className="flex items-center gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <Badge className="bg-green-100 text-white dark:bg-white dark:text-green-200 text-xs">
                          Dépôt autorisé
                        </Badge>
                      </div>
                    </div>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium text-gray-900 dark:text-white">
                          {platform.min_deposit_amount} FCFA
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-500 dark:text-gray-400">Max:</span>
                        <span className="font-medium text-gray-900 dark:text-white">
                          {platform.max_deposit_amount} FCFA
                        </span>
                      </div>
                    </div>
                  </CardContent>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      required
                      min={platform.min_deposit_amount}
                      max={platform.max_deposit_amount}
                    />
                    {!isAmountValid() && formData.amount && (
                      <p className="mt-1 text-sm text-red-600">
                        Montant doit être entre {formatAmount(platform.min_deposit_amount)} et {formatAmount(platform.max_deposit_amount)} FCFA
                      </p>
                    )}
                  </div>
                  ...
                  <Button
                    type="submit"
                    disabled={submitting || !isFormValid() || !isAmountValid()}
                ...
                <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                  <div className="bg-orange-50 dark:bg-orange-900/20 rounded-lg p-4">
                    ...
                    <ul className="text-xs text-orange-700 dark:text-orange-300 space-y-1">
                      <li>• Vérifiez l'ID utilisateur avant de créer le dépôt</li>
                      <li>• Le montant doit respecter les limites de la plateforme</li>
